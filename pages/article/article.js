...
wx.login({
  success: function (res) {
    if (res.code) {
      //使用小程序登录接口完成后端用户登录
      wx.request({
        url: app.globalData.baseUrl+'getopenid',//获取openid session_key api接口的路径
        data: {
          code: res.code,
...
Page({
        ...
        success: function (res) {
          ...
          wx.request({
            ...
            data:{
              ...
              note:that.data.thisarticleinfos.shorttitle,
  ...
  onReady: function () {
    ...
    //品牌列表
    wx.request({
      url: app.globalData.baseUrl+"getsigntemplate/?id="+that.data.id, //请求地址
      method: 'GET',
      dataType: 'json',
      success: function (res) {
        ...
        wx.setNavigationBarTitle({
          title: that.data.thisarticleinfos.title
        });
      },
      fail: function (err) {
        console.log('错误码：' + err.errCode);
